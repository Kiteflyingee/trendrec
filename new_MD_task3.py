...
LAMBDA = 1.0     #超参数LAMBDA
TESTTIME= list(range(1,11))    #TESTTIME 测试集合取的时间长度
...
def massDiffisionForOne(train, user, udegree, idegree, Nitemset, _lambda=1):
    '''
    ...
    users = user_score.sort_values(ascending=False).index
    #进行第三次Mass操作，为每个item进行打分
    for uid in users:
#            获得该user对有关item的打分，用字典表示
        u_itemscore=thirdMass(train, udegree, user_score, uid, Nitemset)
        for item in u_itemscore:
...
def thirdMass(train, udegree, user_score, uid, Nitemset):   
    '''
...
task3 新增代码块
'''
def getTestset(testset , test_ratio):
    '''
    param:
        testset: 测试集 pandas形式
        test_ratio: 测试集比例
    '''
    new_rows = int(testset.shape[0] * test_ratio)
    newtest = testset.sort_values(by=2, axis=0, ascending=True).iloc[0:new_rows,:]
    return newtest
...
def exp(_time):
    ...
    编写实验逻辑
    '''
    # score_filepath = 'temp/md_delicioustemp5k_04.pkl'
    # filepath = r'./data/delicious/delicious5k.pkl'
    # score_filepath = 'temp/md_amazontemp5nok_05_1.pkl'
    score_filepath = 'temp/md_netflixtemp5nok_05_1task3.pkl'
    # filepath = r'./data/Amazon/amazon5k.pkl'
    filepath = r'./data/netflix.pkl'
    # 改为直接加载已经切分排序的数据集
    train_data ,test_data = pickle.load(open(filepath, mode='rb'))
    ...
    # 获得testset item度分布
    newtest = getTestset(test_data, _time)
    test_item_degree = newtest.iid.value_counts()
    ...
    if os.path.exists(score_filepath):
        ...
        for user in tqdm(range(1,train.shape[0]),ascii=True):
            ...
            total_item_score += one_item_score * pow(udegree.get(user), LAMBDA)
    else:
        item_scores = {}
        for user in tqdm(range(1,train.shape[0]),ascii=True):
        # userid从0开始的情况
        # for user in range(train.shape[0]):
            if udegree.get(user, 0.0) == 0.0:
                continue
            one_item_score = massDiffisionForOne(train, user, udegree, idegree, Nitemset, LAMBDA)
            total_item_score += one_item_score * pow(udegree.get(user), LAMBDA)
...
if __name__ == '__main__':
    ...
    scores = []
    # result_path = './result/delicious/md_delicious_user-5k_resultnok.csv'
    result_path = './result/Amazon/task3_md_netflixlambda1.csv'
    with open(result_path,'w',encoding="utf-8") as f:
        for _time in TESTTIME:
            # 因为range的范围是10倍，这里还原
            _time = 0.1 * _time
            corr_score = exp(_time)
