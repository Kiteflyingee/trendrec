...
def massDiffisionForOne(train, user, udegree, idegree, Nitemset, _lambda=1, K=100):
    '''
...
def exp(k, filepath=r'./data/Amazon/amazon5k.pkl'):
    '''
    编写实验逻辑
    '''
    # 改为直接加载已经切分排序的数据集
    ...
    for user in tqdm(range(1, train.shape[0]), ascii=True):
        if udegree.get(user, 0.0) == 0.0:
            continue
        one_item_score = massDiffisionForOne(
            train, user, udegree, idegree, Nitemset, LAMBDA, K=k)
...
if __name__ == '__main__':
    ...
    K = range(50,501,50)
    scores = []
    filepath = r'./data/delicious/delicious5k_rerank.pkl'
    result_path = './result/delicious/task2_md_delicious_user-5k_result_k50-500.csv'
    # filepath = r'./data/Amazon/amazon5k.pkl'
    # result_path = './result/Amazon/md_amazon_user-5k_resultnok.csv'
    # filepath = './data/netflix.pkl'
    # result_path = './result/netflix/netflix_task2_md.csv'
    with open(result_path, 'w', encoding="utf-8") as f:
        for k in K:
            corr_score = exp(k, filepath)
